...
const CourseContent: FC<Props> = ({
  ...
  const newContentHandler = (item: any, index: number) => {
    if (
      item.title === "" ||
      item.description === "" ||
      item.videoUrl === "" ||
      ...
      // add state for new content
      const updatedCollapsed = [...isCollapsed];
      updatedCollapsed.splice(index + 1, 0, false);
      setIsCollapsed(updatedCollapsed);
    }
  };

  const addNewSection = (index: number = courseContentData.length - 1) => {
    if (
      courseContentData[index].title === "" ||
      courseContentData[index].description === "" ||
      courseContentData[index].videoUrl === "" ||
      courseContentData[index].links[0].title === "" ||
      courseContentData[index].links[0].url === ""
    ) {
      toast.error("Please fill all the fields first!");
    } else {
      ...
      // add state for new content
      const updatedCollapsed = [...isCollapsed];
      updatedCollapsed.splice(index + 1, 0, true);
      setIsCollapsed(updatedCollapsed);
    }
  };

  const removeContentHandler = (index: number) => {
    const updatedData = [...courseContentData];
    ...
    <div className="w-[80%] m-auto mt-24 p-3">
      <form onSubmit={handleSubmit}>
          ...
          return (
            <React.Fragment key={index}>
              ...
              >
                ...
                <br />
                {/* add new content */}
                {(index === courseContentData.length - 1 || item.videoSection !== courseContentData[index + 1]?.videoSection) && (
                  <div className="flex flex-col gap-2 mt-4">
                    <p
                      className="flex items-center text-[18px] dark:text-white text-black cursor-pointer"
                      onClick={(e: any) => newContentHandler(item, index)}
                    >
                      <AiOutlinePlusCircle className="mr-2" /> Add New Content
                    </p>
                    <p
                      className="flex items-center text-[18px] dark:text-white text-black cursor-pointer"
                      onClick={(e: any) => addNewSection(index)}
                    >
                      <AiOutlinePlusCircle className="mr-2" /> Add New Chapter
                    </p>
                  </div>
                )}
              </div>
        ...
        <div
          ...
          onClick={() => addNewSection(courseContentData.length - 1)}
        >
