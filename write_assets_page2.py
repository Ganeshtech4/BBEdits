...
export default function AssetsPage() {
  ...
  async function handleDownload(asset: any, e?: React.MouseEvent) {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URI}/get-asset-download/${asset._id}`, { credentials: 'include' });
...
function AssetCard({ asset, onDownload, onPurchase, isDownloading, isPaying }: any) {
  ...
  return (
    <Link href={`/assets/${asset._id}`} className="group relative flex flex-col overflow-hidden rounded-2xl bg-gradient-to-br from-[#110b2e] to-[#0d0720] border border-purple-500/20 shadow-[0_0_20px_rgba(147,51,234,0.06)] transition-all duration-300 hover:-translate-y-1 hover:border-purple-500/50 hover:shadow-[0_0_30px_rgba(147,51,234,0.2)]">
      <div className="relative h-48 overflow-hidden bg-gradient-to-br from-purple-900/30 to-violet-900/20">
        {asset.thumbnail?.url ? (
          <img src={asset.thumbnail.url} alt={asset.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" />
        ) : (
...
with open(r'e:\Work\Clients\bb\anilweb\client\app\assets\page.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
